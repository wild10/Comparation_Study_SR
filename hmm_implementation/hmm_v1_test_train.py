...
def buildDataSet(folder):
    ...
    print("cantidad de files.wav: ",len(fileList))
    dataset = {}
...
def main():
    ...
    print("total de audios: ",len(data_features))
    # print("feature: " , data_features["karina10"])
    ...
    for label in test_data.keys():
        ...
        #print("lista_scores keys: ", lista_scores) lista con probabilidades para cada locutor
        predict = max(lista_scores, key=lista_scores.get)
        print("Test para label ", label, ": resultado de prediccion es: ", predict)
        if predict != label:
            error+=1
    print("error: ", error, " total: ",  total)
    print("tasa de reconocimiento final %.2f"%(100.0*error/total), "%")
